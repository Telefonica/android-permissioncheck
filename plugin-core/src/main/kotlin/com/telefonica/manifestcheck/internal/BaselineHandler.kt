...
import com.telefonica.manifestcheck.internal.util.appendElement
...
import com.telefonica.manifestcheck.internal.util.writeToFile
import java.io.File
...
internal class BaselineHandler(private val baselineFile: File) {
    ...
    fun serialize(permissions: Map<String, Set<BasePermission>>, recreate: Boolean = false) {
        ...
        val document = createDocumentBuilder()
            .newDocument()
    ...
    fun deserialize(): Map<String, Set<BasePermission>> {
        ...
        document.getElementsByTagName("variant").forEach { variant ->
            ...
            sdk23Permissions.forEach { element ->
                val name = element.getAttribute("name")
                val maxSdkVersion = element.getAttribute("maxSdkVersion").toIntOrNull()
                variantPermissions += Sdk23Permission(
                    name,
                    maxSdkVersion
                )
            }
